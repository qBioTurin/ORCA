...
public class Gel_Analyzer implements PlugIn {

    public static final String OPTIONS = "gel.options";
    public static final String VSCALE = "gel.vscale";
    public static final String HSCALE = "gel.hscale";
    public static final int OD=1, PERCENT=2, OUTLINE=4, INVERT=8;
	public static int saveID;
	public static int nLanes, saveNLanes;
	public static Rectangle firstRect;
	public static final int MAX_LANES = 100;
	public static int[] x = new int[MAX_LANES+1];
	public static PlotsCanvas plotsCanvas;
	public static ImageProcessor ipLanes;
	public static ImagePlus gel;
    public	static int plotHeight;
	public static int options = (int)Prefs.get(OPTIONS, PERCENT+INVERT);
	public static boolean uncalibratedOD = (options&OD)!=0;
    public	static boolean labelWithPercentages = (options&PERCENT)!=0;;
	public static boolean outlineLanes;
	public static boolean invertPeaks = (options&INVERT)!=0;
	public static double verticalScaleFactor = Prefs.get(VSCALE, 1.0);
	public static double horizontalScaleFactor = Prefs.get(HSCALE, 1.0);
	...
	public static boolean showLaneDialog = true;

	public void run(String arg) {
	System.out.println("Valore di arg:"+arg);
	System.out.println("Valore di nLanes:"+nLanes);
...
/*
		if (nLanes==0) {
			...
			IJ.register(Gel_Analyzer.class);  // keeps this class from being GC'd
		}
	...
	void showDialog() {
		...
		Panel p= new Panel();
		JButton bPlot = new JButton("Plot Lane(s)");
        ...
        JButton bFLane = new JButton("First Lane");
		bFLane.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
            selectFirstLane(rect);
            System.out.println("F lane selected");
            gd.dispose();
		}
		});
		
        JButton bNLane = new JButton("Next Lane");
		bNLane.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
            if (nLanes==0) {
                show("You must first use the \"Select First Lane\" command.");
                ...
                selectNextLane(rect);
                System.out.println("N lane selected");
            }
            gd.dispose();
		}
		});
	...
	void plotLanes(ImagePlus imp, boolean replot) {
        ...
        Vector<Integer> Limits = new Vector(nLanes);
        ...
        try {
            PrintWriter out = new PrintWriter(f.getSelectedFile() );
            //PrintWriter out = new PrintWriter(file);  
            
            for(int i=1;i<plots.getWidth()-1;i++){ 
                Integer b = 0;
                for(int j=1;j<plots.getHeight()-1;j++){
                    if( j > Limits.get(b) ) {                        
                        b = b+1;
                    }
                    
                    int[] v=plots.getPixel(i,j);
                    String coordString = (i+";"+j+";"+v[0]+";"+b) ;
		...
		plots.show();
		//System.out.println(plots.getResultsTable() );
...
class PlotsCanvas extends ImageCanvas {
	...
	void labelPeaks() {
		...
		for (int i=0; i<counter; i++) {
			Rectangle r = rect[i];
			String s;
			if (Gel_Analyzer.labelWithPercentages)
			...
			if (r.height>=(Gel_Analyzer.plotHeight-11))
				fits = true;
			if (!fits)
				y = r.y - 2;
			ip.drawString(s, x, y);
